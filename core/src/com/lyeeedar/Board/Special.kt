...
abstract class Special(val orb: Orb)
{
	...
	companion object
	{
		...
		fun popColumn(x: Int, sy: Int, grid: Grid)
		{
		...
		fun popRow(sx: Int, y: Int, grid: Grid)
		{
...
abstract class Match4(orb: Orb) : Special(orb)
{
	override fun merge(other: Orb): ((point: Point, grid: Grid) -> Unit)?
	{
		if (other.special != null && other.special is Match4)
		{
			...
			{
				popColumn(point.x, point.y, grid)
				popRow(point.x, point.y, grid)
			}
		}
...
class Horizontal4(orb: Orb) : Match4(orb)
{
	...
	override fun apply() = fun (point: Point, grid: Grid) {	popColumn(point.x, point.y, grid) }
...
class Vertical4(orb: Orb) : Match4(orb)
{
	...
	override fun apply() = fun (point: Point, grid: Grid) {	popRow(point.x, point.y, grid) }
}
	...
	override fun merge(other: Orb): ((point: Point, grid: Grid) -> Unit)?
	{
		...
		if (special != null)
		{
			...
			else if (special is Horizontal4)
			{
				return fun (point: Point, grid: Grid)
				{
					popColumn(point.x-1, point.y, grid)
					popColumn(point.x, point.y, grid)
					popColumn(point.x+1, point.y, grid)
				}
			}
